...
The main issue of why the task remain for the moment unsolved even if it can be solved by PPO is because the environment creation using RK4 is for the moment not parallel. In average 150step per second are rolled while rolling experiment. In comparison system that converge on this task need hundreds million step that can only be achieved through parallel rolling of hundreds or thousands of environment. (meaning at least 150 000 step per second)
